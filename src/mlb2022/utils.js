globalThis.getOpenOrdersTableFromDocument = (document) =>
  document
    .getElementsByClassName("section-block")[0]
...
const addTopBidToRow = (document, row, type) => {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const price = table.rows[1].children[1].innerText
        .replace(",", "")
        .match(/\d+/)[0];
...
const addRunnerUpToRow = (document, row, type) => {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const price = table.rows[2].children[1].innerText
        .replace(",", "")
        .match(/\d+/)[0];
...
const addProfitInfoToRow = (document, row) => {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const buyNowprice = buyNowTable.rows[1].children[1].innerText
        .replace(",", "")
        .match(/\d+/)[0];
      const sellNowprice = sellNowTable.rows[1].children[1].innerText
        .replace(",", "")
        .match(/\d+/)[0];
